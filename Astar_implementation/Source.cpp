...
int aStarSolver(std::vector<std::vector<char>> &map) //function used to solve the maze using A* algorithm
{
	...
	std::vector<node> openList(0);  //structure used to store all the nodes in the open list
	std::vector<node> closedList(0); //structure used to store all the nodes in the closed list
	std::vector<node> adjacentNodes(0); //structure used to store all the adjacent nodes
...
int main(int argc, char *argv[]) //main function 
{
	...
	char all[500] = { 0 }; //store all the chars from the file
	...
	std::vector<std::vector<char>> map(0); //store the map 
