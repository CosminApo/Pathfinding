...
void sdlManager::init(int xResolution, int yResolution) //function to initialise SDL
{
	...
	if (IMG_Init(IMG_INIT_PNG) != 0) //check if images has been initialised
	{
		std::cout << "Unable to initialise images" << IMG_GetError << std::endl;
	}
	else
	{
		std::cout << "Images initialised" << std::endl;
	}
